  ...
  async function deleteAchat(achat) {
    if (confirm("Supprimer cet achat ?")) {
      try {
        ...
        await supabase.from("issaachats").delete().eq("id", achat.id);
        await loadAchats();
      } catch (error) {
            ...
            {achatsList.map((achat) => (
              <tr key={achat.id}>
                ...
                {["Administrateur", "Responsable de production", "Superviseur", "Coordinateur", "Commercial"].includes(
                  ...
                  <td>
                    <NavLink to={`/issaachats/details/${achat.id}`}>
                      <button className="profile">
                        <Eye size={20} />
                      </button>
                    </NavLink>
                    <NavLink to={`/issaachats/modifier/${achat.id}`}>
                      <button className="profile">
                        <Pencil size={20} />
                      </button>
                    </NavLink>
