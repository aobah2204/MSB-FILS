...
function EncaissementCreate() {
  ...
  async function loadData() {
    try {
      ...
      const { data: EncaissementsData } = await supabase.from("encaissements").select("*");
      ...
      const { data: ventesData } = await supabase.from("ventes").select("*, clients(nom, prenom)");
  ...
  async function handleSubmit(e) {
    ...
    try {
      const { data: EncaissementData, error: EncaissementError } = await supabase
        ...
        .insert([
          {
            reference: "MSB_ENC_000"+(Encaissements.length + 1),
        ...
        <div className="card">
          <div>
            ...
            <input
              type="text"
              name="reference"
              value={formData.reference || "MSB_ENC_000"+(Encaissements.length + 1)}
...
export default EncaissementCreate;
